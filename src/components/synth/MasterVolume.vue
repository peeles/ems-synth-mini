<template>
    <SynthPanel title="Stereo Output">
        <div class="flex justify-around items-end h-[140px]">
            ...
            <!-- Left Meter -->
            <div class="relative w-6 h-full bg-gray-300 border border-black rounded-sm overflow-hidden">
                <div v-for="tick in 20" :key="'l' + tick"
                     class="absolute left-0 w-full h-[1px] bg-black/20"
                     :style="{ bottom: `${(tick / 20) * 100}%` }" />
                <div class="absolute bottom-0 left-0 w-full transition-all duration-100"
                     :class="leftColor"
            ...
            <!-- Right Meter -->
            <div class="relative w-6 h-full bg-gray-300 border border-black rounded-sm overflow-hidden">
                <div v-for="tick in 20" :key="'r' + tick"
                     class="absolute left-0 w-full h-[1px] bg-black/20"
                     :style="{ bottom: `${(tick / 20) * 100}%` }" />
                <div class="absolute bottom-0 left-0 w-full transition-all duration-100"
                     :class="rightColor"
        ...
        <!-- Labels -->
        <div class="flex justify-around mt-1 text-[10px] font-mono uppercase tracking-wider">
            <span>L</span>
            <span>R</span>
        </div>
        ...
        <div class="mt-2 text-center text-[10px]">
            <label class="inline-flex items-center space-x-2 cursor-pointer">
                <input type="checkbox" v-model="normalize" class="accent-black" />
